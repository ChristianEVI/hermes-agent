...
function parseDescription(d) {
  ...
  if ((m = lower.match(/(\d+)\s*x\s*([\d.,]+)\s*ml/))) out.fill_volume_ml = parseNum(m[2]);
    ...
    const vol = [
      ...
      /([\d.,]+)\s*ml\s*per\s*(?:pen|vial|syringe|cartridge)/, /([\d.,]+)\s*ml\s*\(\s*[\d.,]+\s*[^)]*\/\s*ml\s*\)/, /-\s*([\d.,]+)\s*ml\b/, /^\s*([\d.,]+)\s*ml\b/, /([\d.,]+)\s*ml\s*(?:solution|suspension|concentrate|emulsion)/
    ];
...
function extractBundle(b, meta) {
  ...
  const productName = nameObj.productName || '';
  const dose_form = parts.dose_form_text || productName;   // aeltere Datensaetze haben keine Namensteile -> Produktname enthaelt die Darreichungsform
  const is_powder = /powder|lyophil/i.test(dose_form);
  const is_oral_solid = /tablet|capsule/i.test(dose_form);
  const is_oral = is_oral_solid || /\boral\b/i.test(dose_form);
  ...
  // Staerke-Text (Namensteil, sonst Produktname): "100 UNITS/ML" macht Insuline/Enzyme zur Einheiten-Familie, auch wenn PMS nur mg liefert
  const tc = parseConcText(parts.strength_text || productName);
  const textUnitsFamily = !!(tc && tc.unit && familyOf(tc.unit) === 'units');
  const anyUnits = ingredients.some(i => familyOf(i.numerator_unit) === 'units') || textUnitsFamily;
  ...
  const primaryLabel = primaryFamily === 'mass' ? 'mg' : (primaryFamily === 'units' ? (mids[0] ? mids[0].numerator_unit : (apds[0] ? apds[0].numerator_unit : (textUnitsFamily ? tc.unit : 'IU'))) : null);
  ...
  const packages = byType('PackagedProductDefinition').map(p => {
    ...
    const hard = [], soft = [];
    const pkg_powder = is_powder || desc.solvent_ml != null || /powder/i.test(p.description || '');
    ...
    else if (!pkg_powder && fill != null && conc != null) { amt = +(fill * conc).toFixed(4); amt_src = 'derived.volume*concentration'; }
    ...
    if (!pkg_powder && !is_oral_solid && fill == null && amt != null && conc) { fill = +(amt / conc).toFixed(4); fill_src = 'derived.amount/concentration'; soft.push('Fuellvolumen aus Menge/Konzentration abgeleitet, SmPC pruefen'); }
    if (pkg_powder) {
    ...
    if (amount_unit === 'mg' && amt != null && amt > 5000 && !is_oral) hard.push('Menge unplausibel (>5 g je Einheit), PMS-Einheit pruefen');
